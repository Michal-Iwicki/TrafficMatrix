...
# KONFIGURACJA STRONY
# ==========================================
st.set_page_config(page_title="Interaktywny Dashboard OD", layout="wide")
...
# Inicjalizacja stanu aplikacji (pamięć stanów Streamlit)
...
def load_and_preprocess_data():
    ...
    segments_daily = segments_raw.groupby(["segment_id", "wkt"], as_index=False).agg(
    ...
    trip_stats = df.groupby("trip_uid").agg(
        ...
        start_lat=("lat", "first"), start_lon=("lon", "first"),
        end_lat=("lat", "last"), end_lon=("lon", "last"),
...
def plot_dynamic_voronoi(voronoi_dynamic, flows_df, top_n):
    colormap = cm.LinearColormap(["#440154", "#3b528b", "#21918c", "#5ec962", "#fde725"], vmin=voronoi_dynamic["events"].min(), vmax=voronoi_dynamic["events"].max())
    m = folium.Map(location=[52.25, 20.98], zoom_start=11, tiles="CartoDB positron")
    
    folium.GeoJson(voronoi_dynamic, style_function=lambda f: {"fillColor": colormap(f["properties"]["events"]), "color": "#333333", "weight": 1.2, "fillOpacity": 0.35},
                   tooltip=folium.GeoJsonTooltip(fields=["terminal_id", "events", "area_km2"])).add_to(m)
    ...
    if not top_flows.empty:
        ...
        for _, row in top_flows.iterrows():
            ...
            folium.PolyLine(locations=[c1, c2], color="#ff2a00", weight=2 + 10 * (w / max_trips), opacity=0.6,
                            tooltip=f"{t1} &harr; {t2}: {w} trips").add_to(m)
...
def plot_od_heatmap(flows_df, unique_terms):
    # Wymuszenie castowania do stringa przed rzutowaniem na int
    all_terms = sorted(unique_terms, key=lambda x: int(str(x).replace('T', '')))
    
    # Obsługa przypadku, w którym brak danych połączonych na mapie
    if flows_df.empty:
        fig, ax = plt.subplots(figsize=(7, 5))
        ax.text(0.5, 0.5, "Brak danych połączeń dla wybranych terminali", ha='center', va='center')
        ax.set_title("Wierszowo Normalizowana Macierz OD")
        plt.tight_layout()
        return fig
        
    # Używamy pivot_table z aggfunc sum zamiast zwykłego pivot (dużo bezpieczniejsze przy duplikatach indeksu)
    od_matrix = flows_df.pivot_table(index="origin_terminal", columns="destination_terminal", values="trips", aggfunc='sum').fillna(0)
    
    # Upewniamy się, że macierz ma pełne wymiary
    od_matrix = od_matrix.reindex(index=all_terms, columns=all_terms, fill_value=0)
    
    # Bezpieczna normalizacja bez wywalania wierszy z zerami
    row_sums = od_matrix.sum(axis=1)
    od_matrix_norm = od_matrix.div(row_sums.replace(0, 1), axis=0)
    
    fig, ax = plt.subplots(figsize=(7, 5))
    sns.heatmap(od_matrix_norm, cmap="mako_r", annot=False, linewidths=0.5, ax=ax, cbar_kws={'label': 'Row-normalized flow'})
    ax.set_title("Wierszowo Normalizowana Macierz OD")
    ax.set_ylabel("Terminal Początkowy (Origin)")
    ax.set_xlabel("Terminal Docelowy (Destination)")
...
# ==========================================
# INTERFEJS UŻYTKOWNIKA (GÓRNY PANEL)
# ==========================================
st.title("🗺️ Interaktywny Dashboard OD & Voronoi")

with st.spinner("Wczytywanie i przetwarzanie danych..."):
    try:
        trip_stats_filtered, cluster_stats, clip_geom = load_and_preprocess_data()
    except Exception as e:
        st.error(f"Nie udało się załadować plików CSV. Błąd: {e}")
        st.stop()

all_clusters = sorted(cluster_stats["endpoint_cluster"].tolist())
cluster_dict = {row["endpoint_cluster"]: f"T{row['endpoint_cluster']} ({row['events']} podróży)" for _, row in cluster_stats.iterrows()}

st.markdown("### ⚙️ Parametry Filtrowania i Generowania")

# Dwie kolumny po równo dla list + jedna na suwak
col_active, col_inactive, col_slider = st.columns([2, 2, 1.5])

with col_active:
    selected_clusters = st.multiselect(
        "🟢 Aktywne terminale (wybierz minimum 3):",
        options=all_clusters,
        default=all_clusters,
        format_func=lambda x: cluster_dict[x]
    )

with col_inactive:
    # Wyliczamy listę odznaczonych elementów
    inactive_clusters = [c for c in all_clusters if c not in selected_clusters]
    st.multiselect(
        "🔴 Nieaktywne terminale (odrzucone):",
        options=inactive_clusters,
        default=inactive_clusters,
        disabled=True, # Tylko dla podglądu stanu wykluczonych
        format_func=lambda x: cluster_dict[x]
    )

with col_slider:
    top_n = st.slider(
        "Liczba najczęstszych tras (Top N linii):",
        min_value=1,
        max_value=100,
        value=15,
        step=1
...
btn_generate = st.button("🚀 Generuj / Odśwież Dashboard", type="primary", use_container_width=True)
...
if st.session_state.generated:
    if len(selected_clusters) < 3:
        st.error("Wybierz przynajmniej 3 aktywne terminale, aby poprawnie wyznaczyć regiony Voronoi!")
    else:
        with st.spinner("Przeliczanie siatki Voronoi i potoków ruchowych..."):
            vor_dyn = rebuild_voronoi(selected_clusters, cluster_stats, clip_geom)
            flows = get_od_flows(vor_dyn, trip_stats_filtered)
            
            col_map, col_matrix = st.columns([3, 2])
            
            with col_map:
                st.subheader("Mapa Regionów i Najczęstszych Tras")
                m = plot_dynamic_voronoi(vor_dyn, flows, top_n=top_n)
                st_folium(m, width=750, height=550, returned_objects=[])
                
            with col_matrix:
                st.subheader("Macierz OD (Origin-Destination)")
                fig = plot_od_heatmap(flows, vor_dyn["terminal_id"].unique())
                st.pyplot(fig)
                plt.close(fig) # Czyszczenie pamięci pyplot po wygenerowaniu (zapobiega nakładaniu się wykresów!)
else:
    st.info("💡 Dostosuj parametry na górze strony i kliknij 'Generuj / Odśwież Dashboard', aby wyświetlić wyniki.")